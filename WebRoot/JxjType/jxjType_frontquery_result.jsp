...
<jsp:include page="../header.jsp"></jsp:include>
		...
		<div class="col-md-9 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	...
			    	<li role="presentation" class="active"><a href="#jxjTypeListPanel" aria-controls="jxjTypeListPanel" role="tab" data-toggle="tab">奖学金类型列表</a></li>
			    	<li role="presentation" ><a href="<%=basePath %>JxjType/jxjType_frontAdd.jsp" style="display:none;">添加奖学金类型</a></li>
				</ul>
			  	<!-- Tab panes -->
			  	<div class="tab-content">
				    <div role="tabpanel" class="tab-pane active" id="jxjTypeListPanel">
				    		<div class="row">
				    			<div class="col-md-12 top5">
				    				<div class="table-responsive">
				    				<table class="table table-condensed table-hover">
				    					<tr class="success bold"><td>序号</td><td>类别id</td><td>类别名称</td><td>奖学金金额</td><td>评定标准</td><td>添加时间</td><td>操作</td></tr>
				    					<% 
				    						/*计算起始序号*/
				    	            		...
				    	            		for(int i=0;i<jxjTypeList.size();i++) {
					    	            		...
					    	            		JxjType jxjType = jxjTypeList.get(i); //获取到奖学金类型对象
 										%>
 										<tr>
 											<td><%=currentIndex %></td>
 											<td><%=jxjType.getTypeId() %></td>
 											<td><%=jxjType.getTypeName() %></td>
 											<td><%=jxjType.getJxjMoney() %></td>
 											<td><%=jxjType.getPdbz() %></td>
 											<td><%=jxjType.getAddTime() %></td>
 											<td>
 												<a href="<%=basePath  %>JxjType/<%=jxjType.getTypeId() %>/frontshow"><i class="fa fa-info"></i>&nbsp;查看</a>&nbsp;
 												<a href="#" onclick="jxjTypeEdit('<%=jxjType.getTypeId() %>');" style="display:none;"><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="jxjTypeDelete('<%=jxjType.getTypeId() %>');" style="display:none;"><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
 										<%}%>
				    				</table>
	...
	<div class="col-md-3 wow fadeInRight">
		...
		<form name="jxjTypeQueryForm" id="jxjTypeQueryForm" action="<%=basePath %>JxjType/frontlist" class="mar_t15" method="post">
...
<div id="jxjTypeEditDialog" class="modal fade" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title"><i class="fa fa-edit"></i>&nbsp;奖学金类型信息编辑</h4>
      </div>
      ...
      <div class="modal-footer"> 
      	...
      	<button type="button" class="btn btn-primary" onclick="ajaxJxjTypeModify();">提交</button>
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
function GoToPage(currentPage,totalPage) {
    ...
    document.jxjTypeQueryForm.currentPage.value = currentPage;
    document.jxjTypeQueryForm.submit();
}

/*可以直接跳转到某页*/
function changepage(totalPage)
{
    var pageValue=document.jxjTypeQueryForm.pageValue.value;
    if(pageValue>totalPage) {
        alert('你输入的页码超出了总页数!');
        return ;
    }
    document.jxjTypeQueryForm.currentPage.value = pageValue;
    documentjxjTypeQueryForm.submit();
}
...
/*删除奖学金类型信息*/
function jxjTypeDelete(typeId) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
			url : basePath + "JxjType/deletes",
			data : {
				typeIds : typeId,
			},
			success : function (obj) {
				if (obj.success) {
					...
					//location.href= basePath + "JxjType/frontlist";
				}
				else 
					alert(obj.message);
			},
