				...
				$.ajax({
					...
					success : function (userInfo, response, status) {
						...
						if (userInfo) { 
							...
							$("#userInfo_colleageObj_collleageId_edit").combobox({
								url:"Colleage/listAll",
							    valueField:"collleageId",
							    textField:"colleageName",
							    panelHeight: "auto",
						        ...
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#userInfo_colleageObj_collleageId_edit").combobox("select", userInfo.colleageObjPri);
									//var data = $("#userInfo_colleageObj_collleageId_edit").combobox("getData"); 
						            //if (data.length > 0) {
						                //$("#userInfo_colleageObj_collleageId_edit").combobox("select", data[0].collleageId);
						            //}
								}
							});
